...
import com.example.tradeLedger.serviceImpl.StrategyParameterSync.DerivedDef;
...
/**
 * The Strategy - Indicator - Parameter model, exercised through the one function
 ...
 */
class StrategyParameterSyncTest {
    ...
    private static IndicatorDef indicator(String name) {
        IndicatorDef def = new IndicatorDef();
        def.setName(name);
        return def;
    }

    private static IndicatorParameter indicatorLink(IndicatorDef indicator, Parameter parameter,
    ...
    private static StrategyParameter strategyLink(Parameter parameter, int order) {
        StrategyParameter link = new StrategyParameter();
        link.setStrategy(new Strategy());
        link.setParameter(parameter);
        link.setDisplayOrder(order);
        link.setRequired(true);
        return link;
    ...
    private static List<DerivedDef> emaCrossover() {
        IndicatorDef crossover = indicator("EMA CROSSOVER");
        return StrategyParameterSync.desiredDefs(
    ...
    @Test
    void strategyGathersItsIndicatorParametersAndItsOwn() {
    ...
    void ownershipDecidesScopeSoDedupStaysIntact() {
        ...
        assertEquals(StrategyParamDef.SCOPE_SIGNAL, defs.get("k").scope());
        assertEquals(StrategyParamDef.SCOPE_SIGNAL, defs.get("d").scope());

        for (String code : List.of("sl", "tp", "quantity", "candle_duration", "trigger_duration")) {
            assertEquals(StrategyParamDef.SCOPE_EXECUTION, defs.get(code).scope(),
    ...
    void perUsageOverridesWinOverCatalogValues() {
        IndicatorDef rsi = indicator("RSI");
        List<DerivedDef> defs = StrategyParameterSync.desiredDefs(
    ...
    void aStrategyWithNoIndicatorStillGetsItsUniversalParameters() {
        ...
        assertTrue(defs.stream().allMatch(def -> StrategyParamDef.SCOPE_EXECUTION.equals(def.scope())));
    }

    @Test
    void aFutureCustomStrategyDerivesThroughTheSameFunction() {
        // Custom Strategy A: two indicators, plus the universal execution knobs.
        // No new code path - the model is generic, EMA Crossover is just data.
        IndicatorDef ema = indicator("EMA");
        IndicatorDef rsi = indicator("RSI");
        ...
        assertEquals(StrategyParamDef.SCOPE_SIGNAL, byKey.get("ema_period").scope());
        assertEquals(StrategyParamDef.SCOPE_SIGNAL, byKey.get("rsi_period").scope());
        assertEquals(StrategyParamDef.SCOPE_EXECUTION, byKey.get("sl").scope());
    }

    @Test
    void derivedKnobsFeedTheExistingValidatorUnchanged() {
        // The bridge: everything the catalog produces is exactly what
        // strategy_param_defs has always carried, so validation, scope splitting
        ...
        for (DerivedDef def : emaCrossover()) {
            ...
            assertTrue(StrategyParamDef.SCOPE_SIGNAL.equals(def.scope())
                            || StrategyParamDef.SCOPE_EXECUTION.equals(def.scope()),
