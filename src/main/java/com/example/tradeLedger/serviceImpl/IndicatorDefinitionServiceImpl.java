...
public class IndicatorDefinitionServiceImpl implements IndicatorDefinitionService {
    ...
    private static final Logger log = LoggerFactory.getLogger(IndicatorDefinitionServiceImpl.class);
    ...
    public IndicatorDefinitionServiceImpl(IndicatorDefRepository indicatorDefRepository,
    ...
    public List<IndicatorDefResponse> list(Boolean active) {
        List<IndicatorDef> defs = active == null
                ? indicatorDefRepository.findAllByOrderByNameAsc()
                : indicatorDefRepository.findByActiveOrderByNameAsc(active);
        return defs.stream().map(this::toResponse).toList();
    }

    @Override
    @Transactional(readOnly = true)
    public IndicatorDefResponse get(UUID id) {
        return toResponse(requireIndicator(id));
    }

    @Override
    @Transactional(readOnly = true)
    public IndicatorDefResponse getByName(String name) {
        IndicatorDef def = indicatorDefRepository.findByName(validator.normalizeIndicatorName(name))
    ...
    @Override
    @Transactional
    public IndicatorDefResponse create(IndicatorDefRequest request) {
        if (request == null) {
        ...
        def = indicatorDefRepository.save(def);
    ...
    @Override
    @Transactional
    public IndicatorDefResponse update(UUID id, IndicatorDefRequest request) {
        IndicatorDef def = requireIndicator(id);
        if (request == null) {
            throw new StrategyValidationException("Request body is required");
        }
    ...
    public void delete(UUID id) {
        ...
        indicatorDefRepository.delete(def);
        log.info("DELETE indicator '{}' id={}", def.getName(), id);
    }

    // -------------------------------------------------------------- helpers

    private IndicatorDef requireIndicator(UUID id) {
        return indicatorDefRepository.findById(id)
    ...
    private IndicatorDefResponse toResponse(IndicatorDef def) {
        return new IndicatorDefResponse(
                def.getId(),
                def.getName(),
