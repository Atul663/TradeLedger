...
| `StrategyParamDef` → `strategy_param_defs` | One row per tunable knob |
...
### `strategy_param_defs` — one row per knob (`StrategyParamDef.java`)
...
## 5. The strategy ↔ indicator ↔ parameter triangle

The authoring link is a **name string inside `strategies.rule_tree`**, and
`IndicatorResolver` is the only class that knows the tree's shape. That tree is
the source of truth; `strategy_indicators` indexes it into real foreign keys on
every save (see [5.1](#51-the-derived-index--strategy_indicators)).
...
{"entry":{"cross_above":[{"ind":"EMA","params":{"period":"$fast"}},
                         {"ind":"EMA","params":{"period":"$slow"}}]}}
                            │                        │
              "ind" matches │                        │ "$fast" matches
              by exact name │                        │ parameter_key
                            ▼                        ▼
                    indicator_defs           strategy_param_defs
                    name = 'EMA'             parameter_key = 'fast'
                    paramSchema:             dataType = int
                      period: int            scope    = signal
                                             default  = 9
```
...
`GET /api/v1/strategies` therefore returns both spellings of the relationship,
index-aligned:

```json
"indicators":   ["EMA"],
"indicatorIds": ["7c9e6679-7425-40de-944b-e07fc1f90ae7"],
"unknownIndicators": []
```

`unknownIndicators` entries have no id by construction. Guards
...
unique business columns:

- indicators `EMA` (`period` 2–300) and `RSI` (`period` 2–100)
- strategy **EMA Crossover** with `system = true`
- knobs `fast` (int, signal, 9), `slow` (int, signal, 21, `gt: fast`),
  `sl_pct` (decimal, execution, 1.5), `tp_pct` (decimal, execution, 3.0)
...
| GET | `/api/v1/strategies?active=&search=` | Returns `StrategyDetailResponse` |
